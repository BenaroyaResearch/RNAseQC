...
#' @param pvars.labs (optional) character vector containing axis labels. Can be generated by running PCA with \code{calc_PCAs}.
...
#' @export
#' @usage \code{
#' plot_PCAs(scores_design_pca, PCs=1:3, pvars.labs=NULL,
...
#'           point_order="random"
#'           )}
plot_PCAs <- function(scores_design_pca, PCs=1:3, pvars.labs=NULL,
                      color_by_var=NULL, color_by_var_levels=NULL, color_var_lab=NULL,
                      my_cols=c("blue","red"), na_col="grey50",
                      pch_by_var=NULL, pch_by_var_levels=NULL, pch_var_lab=NULL, my_pch=NULL,
                      text_by_var=NULL, plot_text_and_pch=FALSE, text_by_var_size=1,
  ...
  if (plot_color_by_var) {
    if (!is.numeric(scores_design_pca[,color_by_var])) {
      if (!is.factor(scores_design_pca[,color_by_var]) | !is.null(color_by_var_levels)) {
        if (is.null(color_by_var_levels)) color_by_var_levels <- as.character(unique(scores_design_pca[,color_by_var]))
        scores_design_pca[,color_by_var] <- factor(scores_design_pca[,color_by_var], levels=color_by_var_levels)
      }
  ...
  if (!exists("plot_points") & (is.null(text_by_var) | plot_text_and_pch)) plot_points <- geom_point(size=3)
  
  if (!is.null(text_by_var)) {
    if (exists("plot_points")) {
      plot_points <- plot_points + geom_text(aes_string(label=text_by_var), size=text_by_var_size)
  ...
  for (i in 1:(length(PCs)-1)) {
    for (j in (i+1):length(PCs)) {
      ...
      if (!is.null(file_prefix)) {
        pdf(file=paste(file_prefix, paste("PC", PCs[i], "_vs_PC", PCs[j], sep=""), file_suffix, sep="."),
